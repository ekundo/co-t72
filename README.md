...
| Номер дискеты и метка в рамке панели | Вывороткой в нижнем крае рамки: в шапке места нет — она ровно на сорок знаков, а панелей две. Номер берётся из таблицы дискет самой ОС, поэтому верен и когда дискету назначили командой «9» мимо CO; метка читается с самой дискеты (её пишет `HDIR /M`). |
