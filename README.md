...
Увести буферы самого CO из окна `A000-DFFF` — это то, что нужно для диска D:, —
пока не вышло: свободной памяти ниже `A000` нет. Каталог диска с большим
оглавлением (у дискеты НЖМД их 128) читается в `4100` и достаёт до `4BFF`,
дальше `5000-5FFF` — ещё один каталожный буфер, а `6000-9FFF` занято буфером
копирования. Проверено на живой машине: при открытом `B:`, назначенном на
дискету винта, по этим адресам лежат записи каталога. Перенос буферов на `4800`
поэтому и отменён: он работал ровно до первого такого диска.
