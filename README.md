...
(и `HDIREN.COM` — то же по-английски), редактор `VDE.COM` с документацией
(`VDE.DOC` и `VDE.QRF` по-русски, авторские английские рядом), сама МикроДОС
и `README.txt`.
...
| СС+7 — выбор дискеты НЖМД | Вместо печати файла — но только если винчестер в Векторе есть; иначе на клавише остаётся штатная печать. В панели открывается список дискет: номер, метка, сколько файлов и сколько занято; стрелки водят курсор, на краю страницы подтягивается следующая, ВК назначает выбранную и перечитывает панель ([`docs/co-hdd-list.md`](docs/co-hdd-list.md)). Кода у списка на килобайт с лишним, а свободной памяти у CO меньше четырёхсот байт, поэтому он дописан в конец самой `CO.COM` и читается оттуда в буфер копирования, когда список открывают. Прежний ввод номера остался на `УС+СС+7`. Дальше как было: команда МикроДОС «9», перечитывание панели. Номер запоминается в `CO.HDD` и применяется при следующем запуске; защита этого файла от записи фиксирует выбор как умолчание. Без НЖМД пусковое восстановление выключается совсем: команда «9» обращается к диску, и на Векторе без НГМД это кончалось ошибкой диска ещё до панелей. |
...
| Мусор на экране при выходе из меню | Открывая меню, CO снимает картинку панелей в буфер копирования и на выходе возвращает её оттуда (`0415`). Снимок занимает буфер целиком, а сборка читает туда же `CO.MNU` — меню само затирало свой снимок. Видно было на одной панели из двух: её половина снимка лежит с `6100`, куда список и читается, у соседней — с `7000` и `9000`. Как и с каталогом архива, чинится выходом, а не переносом списка: из меню CO теперь выходит перерисовкой панели ([`co-src/mnuout.asm`](co-src/mnuout.asm)), а снимок ей не нужен вовсе. Панель при этом не та, где курсор: меню рисуется в соседней, поэтому список перерисовывается с переключённым `B69D`, а шапки — со своим, иначе `0C7E` заодно сменит текущий диск ОС. |
...
| «СС»-«4» — редактор VDE | На этой клавише у CO стоял WordStar (`WSR.COM`), а он заточен под КОИ-7: под T-72 в нём не читается и не набирается русский. Вместо него в выпуске [`VDE`](vde/README.md) — редактор Эрика Мейера с тем же набором команд WordStar, переведённый с Z80 на 8080 и работающий в КОИ-8. Имена запускаемых программ лежат в CO таблицей по 12 байт, длина у `VDE` та же, что у `WSR`, — меняется только имя и подпись в нижней строке — и то и другое прямо в [листинге](co-src/co.asm). |
| Нижняя строка подсказки | Обе строки — обычная и по «СС» — были на 79 знаков при ширине экрана в 80: полоса выворотки не доходила до рамки правой панели ровно на знакоместо, а подписи слипались — «3-Просм.4-Мedit», «2-Атриб.3-ПрК7», — пробела между ними не помещалось. Мешало и другое: напечатав знак в последней колонке, консоль T-72 переводит строку, а подсказка — последняя строка экрана, и он прокручивался (панели уезжали вверх, подсказка садилась на командную строку). Теперь перед подсказкой автоперевод выключается (`ЭСК [ 7 l`) и тут же, последними знаками самой подсказки, включается обратно. Выключенным его оставлять нельзя: меню пользователя по «2» рисует рамку, полагаясь на перевод в конце строки, и без него застревало на первой же строке — меню выходило пустым, а на выходе из него экран забивало мусором. Место под восьмидесятый знак нашлось там, где лежали строки бывшего переключателя знакогенератора (`ЭСК [` и `ЭСК \`): сам переключатель сборка давно заменила своим, а под T-72 `ЭСК [` — ещё и начало команды с параметрами, напечатанная одна она проглотила бы следующий знак. Обе строки лежат в [листинге](co-src/co.asm) готовыми, на всю ширину; разложить подписи заново помогает [`tools/barline.py`](tools/barline.py). |
...
Дописанный код лежит в хвосте образа, а работает по `B75C` — при старте копируется
туда стабом по `40DB`: область по `4100` при загрузке занимает сам образ, и код
там затирается. Не с `B700`, как было сначала: у CO по `B6A8` буфер строки на
128 байт, он достаёт до `B727`, а по `B728` лежит таблица меню пользователя.
Под стеком тесно — от `B75C` до буфера дисковода ОС на `BC00` всего 1188 байт, —
поэтому надпись в рамке туда не едет вовсе: её тело остаётся там, куда его
положила загрузка, и копировщик при старте переносит его на `A660`. Что именно
переносится под стек, стаб берёт из меток листинга (`STKFILE`, `STKEND`).

Тело надписи остаётся там, куда его положила загрузка, а под стек едет только
копировщик: места там 1188 байт, и триста из них на тело — непозволительная
...
туда при старте (переносчик в [`co-src/pusk.asm`](co-src/pusk.asm)). Свободные куски окна найдены слежением за
...
Там же, в хвосте листинга, лежат и накладки, которые при старте едут в окно
ОЗУ ([`co-src/okno.asm`](co-src/okno.asm)): склейка экстентов, обмен через
буфер вне окна, поиск по дискам, кодировки просмотрщика и ещё три. Собираются
они по месту работы, а ложатся в хвост — ассемблер это умеет директивой
`PHASE`. Врезки в саму CO, которые их зовут, — обычные `ПРАВКА`-строки со
ссылкой на метку накладки, так что адреса больше нигде не проставляются
руками. В `tools/` остаётся то, что сборка всё ещё выкладывает питоном:
диск `D:`, дискета НЖМД, пусковые накладки и переносчик.
...
Девяносто один сценарий, от стрелок до копирования, распаковки архива и
...
выходило бы 90%, то есть на 486 команд больше, чем CO исполнила.
