...
| архив и образ | образ системы | оборудование |
|---|---|---|
...
образ системы T-72 и дискета; `.edd` — готовый квазидиск с системой, CO, HDIR
и автозапуском.
Дискета загрузочная: на её системных дорожках та же сборка T-72, что и в `.rom`.

Образ системы (`os-t72X.rom` — его можно зашить в ПЗУ, положить на системные
дорожки дискеты или дать эмулятору) — сборка ImproverX от 15.09.2026, она же лежит
в [`t72/`](t72/). Именно её, а не сборку из репозитория T-72: таблицу состава
оборудования ImproverX завёл позже исходников, и на старой системе CO не
показывает номер дискеты НЖМД и не знает состава Вектора.
...
`FFCC`. Архивы различаются только образом системы, дискетой и образом квазидиска.
...
считается без бита окна вовсе (в выложенных сборках там `SUI 03` и `ORI 010h`
вместо `033h`).
