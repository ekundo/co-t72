...
Четвёртой сборки T-72, `os-t72k` (только квазидиск), в выпуске нет: собирается она
без ошибок, но ни разу не проверялась — ни в эмуляторе, ни на железе. Собрать её
можно тем же `patch-co.sh`, адреса он снимает с живой машины сам.
...
которым ПКК помечает перенаправление вывода в файл. Он общий у всех сборок, потому
что БДОС у них один и тот же; переедет — HDIR просто начнёт останавливаться на
страницах при выводе в файл, и это снимается ключом `/N`.
