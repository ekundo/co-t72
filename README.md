...
постраничный, АР2 прекращает. Число дискет не зашито: программа читает заголовок
винчестера — оттуда же его берёт при старте и сама система, — поэтому на диске
другого размера покажет столько дискет, сколько там размечено.

НЖМД читается напрямую портами, теми же и в том же порядке, что и БСВВ: адресов ОС
в программе нет, и одна сборка годится для всех трёх ПЗУ.

```sh
python3 tools/asm8080.py tools/hdir.asm -o out/HDIR.COM
```
