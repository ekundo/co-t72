...
| Буферы вне подэкранной области | Главное для диска D:. Окно `A000-DFFF` при штатном порте `10h`=`23h` — это страница первого квазидиска, а на время обмена то же окно занимает сам квазидиск; всё, что CO там держал, при работе со вторым КД уезжало внутрь него. Замеры на стенде показали одну область, `A000-A447`, — она переехала на `4800`, а буфер копирования сдвинут с `6100` на `6000`, чтобы его 16-килобайтный кусок кончался ровно на границе окна. Подробности и что осталось нетронутым — в `tools/bufmove.py`. |
...
Карта памяти после правок: образ CO до `4515`, надпись в рамке `4600-471B`,
буферы CO `4800-4FFF`, буфер каталога `5000-5FFF`, буфер копирования `6000-9FFF`,
дописанный код под стеком `B740-BB55`.
