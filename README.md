...
`MEDIT`, отладчик `SID`, `RUND`, `KOD`, `SYSGEN`, `GO` и загрузчики на
магнитофон `SAVE*.COM`. Кроме WordStar (`WSR.COM`, «СС»-«4»): он заточен под
КОИ-7 и под T-72 толком не работает. Собираются образы не с нуля: за основу
