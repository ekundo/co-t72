...
**Сборки не взаимозаменяемы**, но уже не из-за дискового обработчика: его адрес CO
теперь берёт при старте из вектора `E213` и под любую сборку T-72 подстраивается
сам. Зашитой осталась таблица дискет НЖМД — её адрес у каждой сборки свой
(`f` — `EA3E`, `h` — `E9C5`, `hx` — `EBEA`, `k` — `E819`), и от чужой сборки номер
с меткой в рамке панели будут неверными.
