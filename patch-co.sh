...
# 3bb. Надпись в рамке панели (номер дискеты НЖМД и метка) переехала в листинг
#      вместе со своим копировщиком -- co-src/pusk.asm. Здесь остаётся только
#      сшить цепочку: в её слот ляжет пусковая дискеты НЖМД, а сама надпись
#      станет следующей за пробой оборудования.
python3 "$HERE/tools/poke.py" "$OUT/CO.COM" "$(python3 "$HERE/tools/sym.py" HDCHOP)" "$INIT"
INIT=$(python3 "$HERE/tools/sym.py" HDCOPY)
# Дальше пойдёт проба оборудования -- она и всё, что за ней, остаётся по
# адресу загрузки: под стек едет только то, что дописано до этой границы.
KEEP=$(printf '%X' $((0x100 + $(wc -c < "$OUT/CO.COM"))))
