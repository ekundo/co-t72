...
python3 "$HERE/tools/barwide.py" "$OUT/CO.COM"
...
# 3be3. Выход из списка архива по «АР2» возвращал на экран картинку панели из
#       буфера копирования -- а каталог архива сборка читает в тот же буфер
#       (3be2), и на экран ложился мусор. Теперь панель рисуется заново;
#       подробности -- в tools/arcexit.py.
python3 "$HERE/tools/arcexit.py" "$OUT/CO.COM"
...
# 3bi2. Таблицу меню пользователя по B728 CO чистит на 2Eh байт, перебирает
#       на 1Ah записей, а заполняет без всякого предела: CO.MNU на тридцать
#       пунктов пишет за таблицу, в наш код под стеком. Ставим предел по краю.
#       Подробности -- в tools/mnutab.asm.
win mnutab.py
