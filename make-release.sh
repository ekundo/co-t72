...
# Отдельный архив редактора -- для тех, кому нужен один VDE, без CO. Собирается
# тем же заходом: иначе в выпуск легко уезжает вчерашний, собранный до правок.
sh "$HERE/vde/make-dist.sh" "$OUT" >/dev/null
