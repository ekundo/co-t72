...
D_DrvA  EQU 0FFCCh              ; таблица ОС: адрес записи дискеты диска A:
...
FRMCOL  EQU 3-INDENT            ; надпись в рамке идёт с четвёртой колонки
FRMLEN  EQU 34                  ; ... и занимает 34 знака (co-src/hdinfo.asm)
FRMCH   EQU 08Dh                ; горизонталь рамки -- ею же и затираем
INDENT  EQU 1                   ; отступ от левого края панели -- как у архива
WIDTH   EQU 38                  ; знаков в строке: справа такой же отступ
LABW    EQU 24                  ; ... из них под метку
...
PARK    EQU 00FF3h              ; курсор в командную строку, строка 23 колонка 3
; Строки подсказки самой CO: BAR0 подводит курсор в нижнюю строку и выключает
; автоперевод, дальше печатается сама подсказка. Обе -- в co-src/co.asm.
...
; Строка под курсором -- обычная и вывороткой. Шапку панели не трогаем: там
; штатное «Диск X Файлов NN Свободно NNN Кб» той дискеты, что назначена на
; диск сейчас, и пока выбираешь новую, это как раз к месту. Метка, которая в
; строку не влезла, остаётся подрезанной -- ради неё целой строки не жалко.
lplain: LDA  lcur
        DCR  A
        STA  lrow
        ADI  TOPROW
...
lhi:    LDA  lcur
        ...
        LXI  H,lnorm
        RST  3
; Курсор -- в командную строку, туда же, куда его уводит CO в панели: иначе
; он мигает сразу за строкой под курсором, чего в списке файлов не бывает.
; Место в конце строки состояния (23-76) для этого не годится, хоть CO и
; оставляет там курсор, дорисовав рамку: сразу после он уводит его в
; командную строку, а моргает курсор именно там, где встал последним, -- и в
; углу он оказывался вплотную к указателю раскладки «К-8».
        JMP  PARK

; Подсказка на время списка. Штатная в этот миг показывает второй набор -- по
; нему сюда и пришли, -- а из него здесь не работает ничего. Стрелки, ВК и АР2
; CO нигде не подсказывает, они и так у всех в руках; остаётся «6», и она из
; ОСНОВНОГО набора, где на этом месте стоит «6-Имя». Ставим её ровно в ту же
; колонку: выглядит так, будто прочие подписи просто погасли, а рабочая
; осталась на своём месте. Обратно штатную вернёт отрисовка панели на выходе
; (091E печатает подпись внизу через 0E54).
lhint:  LXI  H,BAR0
        RST  3
        LXI  H,lhtxt
        RST  3
...
llb1:   LDA  lcur
        ...
        LXI  D,linbuf
        CALL INSTR
        CALL lclr               ; приглашение с набранным -- убрать
        LDA  linbuf+1
        ORA  A
...
llb9:   JMP  lhi                ; строку под курсором перерисовать

; Стереть приглашение вместе с набранным: командная строка от третьей колонки.
lclr:   CALL PARK
        MVI  B,60
lcl1:   PUSH B
        MVI  C,' '
...
hds1:   STA  lpos+3
        ...
        ORA  L
        RZ                      ; дискет нет вовсе
        CALL lstart             ; открываемся на той дискете, что назначена
        CALL lloadpg
        CALL ldraw
        CALL lhint
        CALL lhi
...
; Открываемся на той странице, где лежит дискета, назначенная сейчас на диск
; панели, и ставим на неё курсор: чаще всего выбирают соседнюю, а не первую.
; Номер берём из таблицы ОС -- той же, что читает надпись в рамке: дискету
; могли назначить и командой «9» мимо CO. Не сошлось (нуль, больше, чем есть,
; таблицы нет вовсе) -- открываемся с первой.
lstart: LDA  hdrive             ; 1 -- A:, 2 -- B:
        DCR  A
        LXI  H,D_DrvA
        JZ   lst1
        INX  H
...
lst2:   MOV  A,L                ; пока остаток не меньше страницы -- отнимаем
        ...
        MOV  B,A
        JMP  lst2
lst3:   MOV  A,L                ; остаток -- позиция курсора на странице
        INR  A
        STA  lcur
        MOV  H,B
        MOV  L,C
