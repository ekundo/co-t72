...
; ссылается код или таблицы клавиш, развёрнуто и снабжено метками. Байтами
; осталось около двух с половиной килобайт -- тексты, шаблоны экрана, таблицы
; перекодировки и пара кусков, которые при разборе рассыпаются в бессмыслицу,
; то есть данные.
;
; Таблицы "клавиша -> обработчик" (05DA, 0622, 0637, 077D, 2F64) разложены по
; записям: код клавиши с названием и адрес обработчика меткой. Устройство у них
; одно, разбирает их подпрограмма по 0562.
;
...
; см. tools/diskvec.py; проверку номера диска правит tools/dsel.py -- иначе
; второй квазидиск уходил бы дисководной веткой.
; ==========================================================================
...
; выбирает дискету НЖМД, см. tools/hddsel.py и tools/hdprobe.py.
...
; СТР в просмотрщике -- переход к строке по номеру.
...
; какой пункт подсвечен сначала. Выбор двигают стрелки, ВК подтверждает.
...
; Курсор в начало нижней строки -- координаты лежат по 2337.
...
; своей подпрограммы, см. tools/koi.asm.
...
L_2070:  RET                      ; 2070 c9
; Поставить курсор: в HL -- пара координат. Печатается ESC Y и они.
...
; ЗАСТАВКА
; ==========================================================================
L_3FB8:  .dbn 13,10,'Версия 2.2.2 от 17.09.2026'
