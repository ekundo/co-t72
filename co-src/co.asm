...
; ПРАВКА (0BEA, 3 байта на месте): счётчик файлов копирования печатался в 11ED
; -- три знака в конце строки, которая начинается подводом курсора по 11E9.
; Строка уехала под стек вместе с подводом (её место в образе заняла полоса
; подсказки во всю ширину), а запись числа осталась по старому адресу: цифры
; ложились в пустое место образа, и счётчик на экране навсегда стоял на
; единице, какой её напечатала шапка.
L_0BEA:  LXI  H,GOTO7N                     ; ПРАВКА счётчик уехал вместе со строкой
...
L_3FB8:  .dbn 13,10,'Версия 2.3.0 от 21.09.2026'    ; ПРАВКА
