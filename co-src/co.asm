...
; из этой подпрограммы сам, см. tools/dsel.asm.
...
; но с оговоркой -- tools/hddsel.asm; хвост прежней ниже так и остался мёртвым.
...
; выбирает дискету НЖМД, см. tools/hddsel.asm и tools/hdprobe.asm.
...
; Трамплин уводит туда же, куда C:, см. tools/dsel.asm.
...
; первым доступным, см. tools/dsel.asm.
...
; возвращает тот же самый -- см. tools/fix16k.asm.
...
; переехало под стек (tools/dsel.asm), хвост с 28AE остался на месте.
L_2870:  JMP  MENU                         ; ПРАВКА меню собирается под стеком
; В освободившееся место лёг переключатель порта квазидиска. Сюда никто не
; приходит: вход 2870 уводит в меню, а подмена CALL 03DE у вызывающих
; разваливает экран -- тупик разобран, см. tools/dsel.asm.
KDPORT:  PUSH PSW                          ; ПРАВКА
         PUSH H                            ; ПРАВКА
         MVI  A,010h                       ; ПРАВКА
         LXI  H,0B673h                     ; ПРАВКА буква текущего диска
         MOV  A,M                          ; ПРАВКА
         CPI  'D'                          ; ПРАВКА
         MVI  A,010h                       ; ПРАВКА порт первого квазидиска
         JNZ  kdput                        ; ПРАВКА
         MVI  A,011h                       ; ПРАВКА ... второго
kdput:   LXI  H,03FAh                      ; ПРАВКА операнды OUT внутри записи
         MOV  M,A                          ; ПРАВКА
         LXI  H,040Ch                      ; ПРАВКА
         MOV  M,A                          ; ПРАВКА
         POP  H                            ; ПРАВКА
         POP  PSW                          ; ПРАВКА
         JMP  03DEh                        ; ПРАВКА и в саму запись
         .ds  29                           ; ПРАВКА остаток прежнего меню
...
; пересчитывается перед выводом -- см. tools/koi.asm.
