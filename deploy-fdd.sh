...
# v06x держит образ дискеты в памяти и сбрасывает его в файл при первой же
# записи -- всё, что мы допишем в файл при живом эмуляторе, он затрёт своей
# копией. Один раз я так снёс рабочую дискету, больше не хочу.
if pgrep -f "v06x .*$(basename "$FDD")" >/dev/null 2>&1; then
    echo "эмулятор держит $(basename "$FDD") -- закрой окно, иначе он затрёт запись" >&2
    exit 1
fi
cp "$FDD" "$FDD.bak"
