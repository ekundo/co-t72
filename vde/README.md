...
Прокрутка вниз так не делается: обратного сдвига консоль не умеет, `InsL`
остаётся пустой, и там по-прежнему полная перерисовка.

Проверено на стенде обе стороны. Вверх: точка останова на записи в порт 3
срабатывает при прокрутке, и стек в этот момент VDE-шный. Вниз: экран
получается верный, но перерисовку видно глазом — она ползёт сверху вниз, и
снимок, сделанный в этот момент, застаёт вперемешку новые верхние строки и ещё
старые нижние.
