...
В [`doc/`](doc) лежат руководство и краткая справка по-русски — про эту самую
сборку, с её клавишами и с оговорками про T-72. В репозитории они обычные
тексты UTF-8; на диски их кладёт `patch-co.sh` файлами `VDE.DOC` и `VDE.QRF`,
перекодировав в КОИ-8 (`tools/mktxt.py`). Справка лежит на обоих дисках,
руководство — только на дискете: квазидиск всего 256 КБ, и 28 КБ руководства
съедали бы больше половины того, что на нём остаётся свободным. Авторские
английские `VDE266.DOC` и `VDE266.QRF` из выпуска 2.66 лежат на дискете
рядом.
...
vde/make-dist.sh    # выпуск: release/vde-t72.zip
```

В архиве `VDE.COM`, заметка, карточка клавиш и документация — наша русская и
авторская английская: комплекта у VDE нет, вся настройка лежит внутри самой
программы.

Отдельный архив — для тех, кому нужен один редактор. В выпуск CO редактор
входит и так: `make-release.sh` собирает его сам и кладёт в каждый комплект, а
`patch-co.sh` пятым аргументом ставит его на клавишу «СС»-«4» вместо WordStar
(имя программы и подпись в строке подсказки -- в `co-src/co.asm`).
...
| `^X`, `^Y`, `^Z` против стрелок | 18h — это и стрелка вправо, и вордстаровское «вниз». `AdjKey` сверяет свою таблицу и вордстаровскую вперемешку, по паре на шаг, и `^X` стоит на шаг раньше «вправо» — стрелка уводила курсор вниз. Различить их нельзя, на Векторе это один код, поэтому он отдан стрелке: она клавиша, а не привычка. «Вниз» осталось на своей стрелке, «вправо» у WordStar — `^D`. По той же причине недоступны `^Y` (стереть строку) и `^Z` (сдвинуть экран вперёд): их коды 19h и 1Ah шлют стрелки вверх и вниз. `^Y` перенесена на клавишу «СТР» (1Fh): её код в таблице разбора не значился и пропадал впустую, а название совпало с действием. Правка — один байт кода в таблице, обработчик и длина те же. |
...
| Меню без выворотки | `MsgDsp` печатает сообщения вывороткой, и меню во весь экран так читались тяжело. Оба места, где они рисуются, переведены на свой `zMnuDsp` — тот же вывод, но без `MakAlt`. Шапка, подсказки и вопросы выворотку сохранили. |
