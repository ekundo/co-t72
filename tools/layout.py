...
Печатается таблица и проверяются четыре вещи: куски не налезают друг на
друга, каждый лежит там, где ему положено, запас до соседа не ушёл в минус, а
накладки окна не доросли до надписи в рамке панели.
...
LABEL = 0xA660                        # надпись в рамке панели -- накладкам сюда нельзя
...
PIECES = [
    ...
    ('окно', 'MNUTAB', 'OKNOEND', 'предел таблицы меню'),
        ...
        for where, addr, length, name in win:
            ...
            print('  %04X-%04X  %-28s дыра %04X-%04X, свободно ещё %d байт'
                  % (addr, addr + length - 1, name, h[0], h[1],
                     h[1] - (addr + length - 1)))
        if sym['OKNOEND'] > LABEL:
            bad.append('окно: накладки доросли до %04X, а с %04X надпись '
                       'в рамке' % (sym['OKNOEND'], LABEL))
