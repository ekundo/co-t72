...
# начинаются списки панелей -- по 680h на каждую. Снизу сюда дотягивался сам
# CO: списки CO.MNU и CO.EXT он читал с A000 и без предела по длине. Теперь
# они читаются в буфер копирования (tools/extbuf.asm), и с A000 остаётся один
# CO.ZGR -- два десятка строк. Подробности и как перемерить -- в
# docs/co-buffers.md.
HOLES = [
    (0xA1ED, 0xA7FF),
]
