...
это до входа в неё -- у вызывающих. Вызовы записи приходят из 2A18, 2E4F, 3ABA и
3F59; там и надо ставить порт по диску, для которого идёт операция.
...
def build_ports(at):
    """Подключение и отключение квазидиска с оглядкой на текущий диск.

    Живёт в месте, освободившемся от меню, и исполняется прямо там. Флаги на
    выходе -- от XRA A, как было у оригинальной пары команд; OUT их не трогает.
    """
    a = Asm(at)
    a.label('kdon')
    a.word(0x3A, DRIVE); a.db(0xFE, 0x44)          # LDA drive / CPI 'D'
    a.ref(0xCA, 'on11')
    a.db(0xAF, 0xD3, 0x10, 0xC9)                   # XRA A / OUT 10h / RET
    a.label('on11')
    a.db(0xAF, 0xD3, 0x11, 0xC9)                   # XRA A / OUT 11h / RET
    a.label('kdoff')
    a.word(0x3A, DRIVE); a.db(0xFE, 0x44)
    a.ref(0xCA, 'off11')
    a.db(0x3E, 0x23, 0xD3, 0x10, 0xC9)             # MVI A,23h / OUT 10h / RET
    a.label('off11')
    a.db(0x3E, 0x23, 0xD3, 0x11, 0xC9)
...
def main():
    ...
    # BANK_ON/BANK_OFF пока не патчим -- см. шапку: вызовом туда не зайти.
