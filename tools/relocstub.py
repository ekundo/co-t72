...
делом обработчик СС+7, и он после этого уводил Вектор в мусор. Ловилось это как
...
STUB = 0x40DB
FILE_AT = 0x4100
# Куда переносится хвост. Ниже -- буферы самого CO: по B728-B755 меню
# пользователя держит свою таблицу и чистит её при каждом открытии (39D7), а
# до B727 достаёт буфер строки. Отсюда и B756: раньше хвост начинался с B740,
# и меню съедало первые 22 байта склейки экстентов -- после меню первое же
# перечитывание каталога с файлом длиннее 16 КБ уводило CO в затёртый код.
RUNTIME = layout.STACK_LO
...
def main():
    ...
    p.add_argument('--from', dest='src', help='откуда переносить (по умолчанию 4100)')
    args = p.parse_args()

    d = bytearray(open(args.infile, 'rb').read())
    file_at = int(args.src, 16) if args.src else FILE_AT
    ...
    stub = bytes([0x21, file_at & 0xFF, file_at >> 8,      # LXI H,откуда
