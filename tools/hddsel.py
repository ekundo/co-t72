...
Номер запоминается в файле CO.HDD. Именно в своём файле, а не в хвосте CO.PRM:
CO пишет CO.PRM целиком своей копией и затирает там что угодно чужое.

Файл всегда на C: -- на квазидиске, где лежит система и автозапуск. Раньше диск
брался из ячейки 0004, то есть «текущий на момент старта CO», и это выходило
боком: стоило запустить CO, когда текущим был A:, и номер уезжал на дискету
НЖМД. Причём пишется он уже ПОСЛЕ команды 9, то есть на вновь выбранную
дискету, -- копии расползались по винчестеру, а на C: оставалась прежняя, и
следующий запуск восстанавливал позапрошлый номер.
...
HOMEDRV = 3            # диск для CO.HDD: C:, квазидиск с системой и автозапуском
...
def build(at):
    ...
    # CO.HDD всегда под нулевым пользователем. БДОС берёт номер пользователя из
    # своего состояния, а не из ФУБ: под ненулевым файл и создавался бы с этим
    # номером, и следующий запуск под другим его просто не нашёл бы. Поэтому обе
    # точки входа обёрнуты -- запомнить номер, встать на нулевого, вернуть как
    # было. A и флаги через PUSH PSW: prmload отдаёт «нечего брать» именно ими.
    a.label('usr0')                                                # запомнить и на 0
    a.db(0x0E, 0x20, 0x1E, 0xFF); a.word(0xCD, BDOS)               # БДОС 20h, E=FF
    a.ref(0x32, 'usrsave')
    a.db(0x0E, 0x20, 0x1E, 0x00); a.word(0xCD, BDOS)
    a.db(0xC9)

    a.label('usrback')                                             # вернуть прежний
    a.db(0x0E, 0x20); a.ref(0x3A, 'usrsave'); a.db(0x5F)           # MOV E,A
    a.word(0xCD, BDOS); a.db(0xC9)

    a.label('prmload')
    a.ref(0xCD, 'usr0'); a.ref(0xCD, 'prmload0')
    a.db(0xF5); a.ref(0xCD, 'usrback'); a.db(0xF1, 0xC9)

    a.label('prmsave')
    a.ref(0xCD, 'usr0'); a.ref(0xCD, 'prmsave0')
    a.db(0xF5); a.ref(0xCD, 'usrback'); a.db(0xF1, 0xC9)

    a.label('prmload0')                                            # A=0, если нечего брать
    ...
    a.label('homedrv'); a.db(HOMEDRV)
    a.label('usrsave'); a.db(0)
...
# Нижняя строка подсказки второго набора. Переименования идут через
# tools/barline.py: он разбирает строку на подписи и раскладывает их заново
# на ширину экрана, поэтому длина не зависит от того, насколько подпись
# изменилась. Раньше её приходилось держать вручную -- «Атрибуты» ровно на
# столько же длиннее, на сколько «Hdd» короче «Печати», -- и на Векторе без
# НЖМД, где «Печать» остаётся, «Атриб.» оставалось слипшимся с «3-ПрК7».
#
# Строк две. Первая -- для Вектора с винчестером: на «7» выбор дискеты НЖМД.
# Вторую hdprobe.py кладёт обратно, если система винчестера не нашла; там на
# «7» по-прежнему печать.
RENAME = [('2-Атриб.', '2-Атрибуты'), ('7-Печать', '7-Hdd'), ('0-Выйти', '0-Выход')]
PLAIN = [('2-Атриб.', '2-Атрибуты'), ('0-Выйти', '0-Выход')]
...
def main():
    ...
    # код работает не там, где лежит: в файле он в хвосте, а исполняется под
    # стеком CO, с B740 (почему не с B700 -- см. relocstub.py). В хвосте
    # держать нельзя, 4100 это буфер каталога самого CO, он его затирает при
    # чтении чужого диска.
    if len(d) % 128:
    ...
    # Заодно поправить нижнюю строку второго набора.
    beg = d.find('1-40/60'.encode('koi8-r'))
    if beg < 0:
        sys.exit('не нашёл строку клавиш второго набора -- незнакомый выпуск CO')
    end = d.index(b'\0', beg)
    orig = bytes(d[beg:end])
    bar, missing = barline.relabel(orig, RENAME, len(orig))
    if missing:
        sys.exit('в строке клавиш нет подписи «%s»' % missing[0])
    was_bar, _ = barline.relabel(orig, PLAIN, len(orig))
    d[beg:end] = bar
    d += code
