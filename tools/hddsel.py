...
Панель после смены дискеты не перечитывается: обновить её надо клавишей
«7-Диск». Передавать управление штатному обработчику `2870` пробовал -- он
тянет за собой состояние, которого тут нет, и печатает «Неверное имя».
