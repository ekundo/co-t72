...
REGIONS = [
    (0x6000, 0x9FFF, 'буфер копирования, чтения файлов и обмена с квазидиском, '
     'он же копия картинки панелей',
     '3216: LXI B,4000h -- шестнадцать килобайт с 6100; туда же кладут 15D5, '
     '161C и 18EE, а с 9100 собирается текст CO.MNU (3BE2). Картинку панелей '
     'CO держит здесь же и возвращает её на экран (0415): по четверти на '
     'половину экрана -- 0423, 0431, 043C, 0447 и 0452 называют 9FFF, 8FFF, '
     '6FFF и 7FFF, а перепись идёт вниз на 10h страниц (0426, 0434, 044A). '
     'Из-за этой копии здесь нельзя держать ничего, что должно пережить '
     'просмотр, -- на этом обожглись каталогом архива, см. tools/arcexit.py'),
    ...
    # Эту границу листингом не вывести: CO пишет сюда через указатель, и ни
    # одного адреса из области не называет. Взята прогоном -- снимком памяти
    # после старта CO (V06X_RAM_SAVE): к моменту, когда появляются панели,
    # заполнено ровно до A447, дальше нули. Проверено и обратным способом:
    # накладки, сдвинутые с A448 на A200, CO до панелей не доводят вовсе.
    (0xA200, 0xA447, 'разобранные строки CO.EXT и CO.MNU',
     'снимок памяти после старта: имена вида «C:MEDIT», «RUN B:AWTORAL.B», '
     '«C:CO.PRM» лежат до A447 включительно'),
