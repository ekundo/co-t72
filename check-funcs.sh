...
# Накладка «дискета НЖМД» (co-src/hddsel.asm) живёт по адресу, который выбирает
...
# Метка у первой -- её показывает рамка панели (co-src/hdinfo.asm). Собирается
...
# Третий образец -- на много дискет: в заголовке их двадцать пять, а данных
# на диске только под две. Дальние строки список покажет пустыми, и это ровно
# то, что нужно сценарию листания: страниц становится две, а образ остаётся
# маленьким -- настоящий на двадцать пять дискет весит двадцать мегабайт, и
# копировать его на каждый прогон незачем.
HDDMANY=$TMP/винт-много.hdd
python3 "$HERE/tools/mkhdd.py" create "$HDDMANY" --disks 2 >/dev/null
python3 "$HERE/tools/mkhdd.py" label "$HDDMANY" 1 'ПРОВЕРКА' >/dev/null
python3 - "$HDDMANY" <<'PY4'
import sys
...
probe() {   # имя, обработчик, клавиши для keytyper, [чужой]
    ...
    case "$alien" in
        ...
        винт-много) alien=; arc=; conly=1; hdd=3 ;;
...
EOF
    ...
    if [ -n "$hdd" ]; then
        case "$hdd" in
            2) cp "$HDDBAD"  "$TMP/$name.hdd" ;;
            3) cp "$HDDMANY" "$TMP/$name.hdd" ;;
...
# этого не годится: вход переставлен на стаб переносчика (40DB).
...
# CO на двух одинаковых строках лезет в соседние (см. co-src/fix16k.asm).
rows = 'нет_снимка'
if alien:
...
PY
    ...
    if [ "$name" = списокнжмд ] && [ -f "$TMP/$name.ram" ]; then
        if python3 - "$TMP/$name.ram" <<'PY3'
import sys
r = open(sys.argv[1], 'rb').read()
rows = [r[0xA954 + i * 13:0xA954 + i * 13 + 12] for i in range(r[0xB689])]
sys.exit(0 if any(n.startswith(b'CO ') and n.endswith(b'HLP') for n in rows) else 1)
PY3
        then
            echo "  список дискет: выбрана вторая, её каталог в панели"
        else
            echo "  список дискет: выбор стрелкой не сработал" >&2
            bad=$((bad+1)); ok=$((ok-1))
        fi
    fi
    # Листание: ВК на первой строке второй страницы выбирает дискету 14h, а
    # выбранное CO записывает в C:CO.HDD тем же набором, каким его набрали бы
    # руками. Нашёлся набор -- значит листание довело до второй страницы.
    if [ "$name" = листание ] && [ -f "$TMP/$name-out.edd" ]; then
        if python3 "$HERE/tools/kdimg.py" get "$TMP/$name-out.edd" CO.HDD \
               "$TMP/$name.prm" >/dev/null 2>&1 &&
           grep -qa '0014' "$TMP/$name.prm"; then
            echo "  список дискет: листание довело до второй страницы"
        else
            echo "  список дискет: листание до второй страницы не довело" >&2
            bad=$((bad+1)); ok=$((ok-1))
        fi
    fi
    # Дисковод первой строкой: выбрали его -- в панели каталог дискеты, а на
    # ней есть то, чего на винчестере нет.
    if [ "$name" = дисковод ] && [ -f "$TMP/$name.ram" ]; then
        if python3 - "$TMP/$name.ram" <<'PY4'
import sys
r = open(sys.argv[1], 'rb').read()
rows = [r[0xA954 + i * 13:0xA954 + i * 13 + 12] for i in range(r[0xB689])]
sys.exit(0 if any(n.startswith(b'VDE266') for n in rows) else 1)
PY4
        then
            echo "  список дискет: дисковод выбран, в панели дискета"
        else
            echo "  список дискет: дисковод на диск панели не встал" >&2
            bad=$((bad+1)); ok=$((ok-1))
        fi
    fi
...
PY2
    ...
    if [ "$name" = меткасписком ] && [ -f "$TMP/$name.hdd" ]; then
        if python3 "$HERE/tools/mkhdd.py" list "$TMP/$name.hdd" 2>/dev/null \
           | grep -q 'метка «LST»'; then
            echo "  список дискет: метка по «6» легла на винчестер"
        else
            echo "  список дискет: метка по «6» на диск не легла" >&2
            bad=$((bad+1)); ok=$((ok-1))
        fi
    fi
...
# CO отдала управление чужой программе; какой -- видно по началу отрисовки
# экрана редактора (Orient). Адрес берём из его же листинга: от любой правки в
# VDE он уезжает, и зашитый здесь молча превратил бы проверку в ложное «да».
VDEORI=$(sed -n 's/^\([0-9A-F]\{4\}\).*Orient:.*/\1/p' "$HERE/work/vde/vde.lst" 2>/dev/null | head -1)
[ -n "$VDEORI" ] || { echo "нет адреса Orient -- собери редактор (vde/build.sh)" >&2; exit 1; }
probe редакторvde "$VDEORI" '"Down", 40, "Down", 40, "\001Left Shift", 10, "4", 20, "\002Left Shift", 1200' чужой
...
# держит картинку панелей (правка по 2DC3 в листинге). В памяти этого не видно, и
...
# co-src/viewtop.asm: без него откат на 13h строк уезжал за начало файла и CO
# вис навсегда.
...
# СС+7 без винчестера остаётся штатной печатью файла: проба оборудования возвращает
...
# «СС»-«7» открывает список дискет, ВК на строке назначает. Первая строка
# списка -- не дискета, а сам дисковод (номер 0000), поэтому до дискеты 1, на
# которой лежит HDIR.COM, надо шагнуть стрелкой.
probe ссемь    "$HDSEL" '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 400, "Down", 200, "Return", 900' винт
# Две стрелки и ВК -- дискета 2, на ней CO.HLP. Этим проверяется и сам список:
# чтение винчестера портами, сбор страницы, курсор.
probe списокнжмд "$HDSEL" '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 400, "Down", 100, "Down", 200, "Return", 900' винт
# Листание страниц: «вправо» уводит на вторую страницу, «влево» возвращает.
# На образце двадцать пять дискет, то есть страницы две; ВК в конце выбирает
# ту, на которой стоит курсор, -- первую строку второй страницы, дискету 14h.
# Она за краем данных образца, поэтому каталог пуст, а нам важно, что выбор
# дошёл до дискеты со второй страницы: номер ложится в таблицу ОС.
probe листание "$HDSEL" '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 400, "Right", 250, "Left", 250, "Right", 250, "Return", 900' винт-много
# ВК на самой первой строке вешает на диск панели дисковод. Проверяется по
# делу: в панели должен оказаться каталог дискеты, а на ней есть VDE266.DOC,
# которого на винчестере нет.
probe дисковод "$HDSEL" '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 400, "Return", 900' винт
# Метка прямо из списка, по «6». Вторая дискета образца метки не имеет -- ей и
# ставим. Это второй сценарий, где стенд пишет на винчестер (первый -- HDIR /M).
probe меткасписком "$HDSEL" '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 400, "Down", 100, "Down", 250, "6", 250, "L", 40, "S", 40, "T", 40, "Return", 700' винт
...
probe ошибкадиска 18FB '"Tab", 100, "\001Left Shift", 10, "7", 20, "\002Left Shift", 250, "Down", 60, "Return", 900, "3", 500, "Return", 400' винт-битый
