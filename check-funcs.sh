...
EOF
    ...
    if [ -n "$arc" ]; then
        ...
        # свободного места. С тех пор как на квазидиск лёг редактор, столько
        # там не набирается. В распаковке он не участвует -- убираем его из
        # копии, а не с самого образа: остальные сценарии гоняются по тому же
        # диску, какой уезжает в выпуск.
        for f in VDE.COM VDE266.QRF; do
