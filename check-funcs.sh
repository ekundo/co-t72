...
#
# Вторая мерка -- подэкранная область A000-DFFF. После переноса буферов (см.
# tools/bufmove.py) CO не должен класть туда ничего: именно это и делает
# возможной работу со вторым квазидиском. Смотрим записи обменных подпрограмм
# ОС; DFC9-DFFF не в счёт -- это её собственный хвост, он был и до правок.
#
...
# сняты с живой машины -- слежением за ячейкой B693, куда CO кладёт код нажатой
# клавиши:
#
...
ROM=${2:-$HERE/tools/MDOS_T-72/BIN/os-t72hx.rom}
RUN=$HERE/run
OUT=$(cd "$OUT" && pwd)
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
...
printf '%-14s %-8s %-4s %-10s %s\n' функция обработчик было окно адресов
printf '%-14s %-8s %-4s %-10s %s\n' -------------- -------- ---- ---------- --------
